...
func TestPodBunchInfo_NewPodBunchInfo(t *testing.T) {
	type args struct {
		uid         BunchID
		job         JobID
	...
	tests := []struct {
	...
	}{
		{
			...
			args: args{
				uid: BunchID("test-uid"),
				job: JobID("test-job"),
				policy: &scheduling.BunchPolicySpec{
					BunchSize: int32Ptr(4),
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("NewPodBunchInfo(%v, %v, %v, %v) got = %v, want %v", tt.args.uid, tt.args.job, tt.args.policy, tt.args.matchValues, got, tt.want)
			}
		})
	}
}
